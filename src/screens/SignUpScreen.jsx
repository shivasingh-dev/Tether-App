...
import { stepOneSchema, stepTwoSchema, stepThreeSchema, stepFourSchema, loginSchema } from '../YupSchema/YupSchema'
...
export default function SignUpScreen({ navigation }) {
  ...
  const stepThreeFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await sendEmailOtpFun(values.email, values.password, phoneNumber)
        if (response?.success) {
  ...
  const currentStep = !sendPhoneOtp ? 1 : !isVerifyPhoneOtp ? 2 : !sendEmailOtp ? 3 : 4
  ...
  const handleResendEmailOtp = async () => {
    ...
    try {
      setIsLoading(true)
      await sendEmailOtpFun(email, stepThreeFormik.values.password, phoneNumber)
      setEmailOtp(['', '', '', '', '', ''])
  ...
  const handleEmailOtpChange = (val, index) => {
    const refs = [emailOtp1Ref, emailOtp2Ref, emailOtp3Ref, emailOtp4Ref, emailOtp5Ref, emailOtp6Ref]
  ...
  return (
    <SafeAreaView style={styles.outermost}>
      ...
      {signUp && (<>
        ...
        <View style={styles.progressRow}>
          {[1, 2, 3, 4].map(s => (
            <View
              key={s}
              style={[styles.progressDot, { backgroundColor: s <= currentStep ? '#2979ff' : '#2E3A59' }]}
            />
          ))}
        </View>
        ...
        {!sendPhoneOtp && (
          <View>
            ...
            <Text style={styles.detailsText}>Enter your details to get started</Text>

            <Text style={styles.formText}>Full Name</Text>
            <View style={[styles.inputRow, { borderColor: isFocused === 'name' ? '#2979ff' : '#2E3A59' }]}>
            ...
            {stepOneFormik.touched.fullName && stepOneFormik.errors.fullName && (
              <Text style={styles.errorText}>{stepOneFormik.errors.fullName}</Text>
            )}

            <Text style={styles.formText}>Phone Number</Text>
            <View style={[styles.phoneRow, { borderColor: isFocused === 'phone' ? '#2979ff' : '#2E3A59' }]}>
            ...
            {stepOneFormik.touched.phoneNumber && stepOneFormik.errors.phoneNumber && (
              <Text style={styles.errorText} >{stepOneFormik.errors.phoneNumber}</Text>
            )}

            <TouchableOpacity activeOpacity={0.4} disabled={isLoading || !stepOneFormik.isValid} onPress={stepOneFormik.handleSubmit}>
              <LinearGradient
                style={[styles.otpCover, (isLoading || !stepOneFormik.isValid) && { opacity: 0.5 }]}
              ...
              >
                ...
                </View>) : <Text style={styles.otpBtn} >Send OTP</Text>}
              </LinearGradient>
            </TouchableOpacity>

            <Text style={[styles.formText, { marginTop: 25, textAlign: 'center' }]}>
              Already have an account?{' '}
              <Text style={styles.link} onPress={() => setSignUp(false)} >Login here</Text>
            </Text>
          </View>
        )}
        ...
        {sendPhoneOtp && !isVerifyPhoneOtp && (
          <View>
            <TouchableOpacity onPress={handleBackFormStepTwo} style={styles.backBtn}>
            ...
            <View style={styles.otpBoxRow}>
              {[otp1Ref, otp2Ref, otp3Ref, otp4Ref, otp5Ref, otp6Ref].map((ref, i) => (
                <TextInput
                  key={i}
                  ref={ref}
                  style={[styles.otpBox, { borderColor: phoneOtp[i] ? '#2979ff' : '#2E3A59' }]}
                  maxLength={1}
                  keyboardType="numeric"
                  value={phoneOtp[i]}
            ...
            {stepTwoFormik.errors.phoneOtp && (
              <Text style={styles.errorText}>{stepTwoFormik.errors.phoneOtp}</Text>
            )}

            <TouchableOpacity disabled={isLoading} activeOpacity={0.4} onPress={stepTwoFormik.handleSubmit}>
              <LinearGradient
              ...
              >
                ...
                </View>) : <Text style={styles.otpBtn} >Verify OTP</Text>}
              </LinearGradient>
            </TouchableOpacity>

            <TouchableOpacity disabled={!canResend} onPress={handleResendPhoneOtp} style={{ marginTop: 16, alignItems: 'center' }}>
        ...
        {isVerifyPhoneOtp && !sendEmailOtp && (
          <View>
            <TouchableOpacity onPress={handleBackFormStepThree} style={styles.backBtn}>
              <Feather name="arrow-left" size={20} color="#2979ff" />
              <Text style={styles.backText}>Back</Text>
            </TouchableOpacity>
            ...
            <Text style={styles.detailsText}>This email will be used to log in on web</Text>

            <Text style={styles.formText}>Email</Text>

            <View style={[styles.inputRow, { borderColor: isFocused === 'email' ? '#2979ff' : '#2E3A59' }]} >
              ...
              <TextInput
                ...
                style={styles.inputInner}
              /></View>
            {stepThreeFormik.touched.email && stepThreeFormik.errors.email && (
              <Text style={styles.errorText}>{stepThreeFormik.errors.email}</Text>
            )}

            <Text style={styles.formText}>Password</Text>
            <View style={[styles.passwordRow, { borderColor: isFocused === 'password' ? '#2979ff' : '#2E3A59' }]}>
            ...
            {stepThreeFormik.touched.password && stepThreeFormik.errors.password && (
              <Text style={styles.errorText}>{stepThreeFormik.errors.password}</Text>
            )}

            <Text style={styles.formText}>Confirm Password</Text>
            <View style={[styles.passwordRow, { borderColor: isFocused === 'confirm' ? '#2979ff' : '#2E3A59' }]}>
              <Feather name='lock' size={20} color="#888" />
              <TextInput
            ...
            {stepThreeFormik.touched.confirmPassword && stepThreeFormik.errors.confirmPassword && (
              <Text style={styles.errorText} >{stepThreeFormik.errors.confirmPassword}</Text>
            )}

            <TouchableOpacity disabled={isLoading || !stepThreeFormik.isValid} activeOpacity={0.4} onPress={stepThreeFormik.handleSubmit}>
              <LinearGradient
                style={[styles.otpCover, (isLoading || !stepThreeFormik.isValid) && { opacity: 0.5 }]}
                colors={['#2979ff', '#7c3aed']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
              >
                {isLoading ? (<View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }} >
                  <ActivityIndicator size="small" color="#fff" style={{ marginRight: 10 }} />
                  <Text style={styles.otpBtn} >Sending</Text>
                </View>) : <Text style={styles.otpBtn} >Send Email OTP</Text>}
              </LinearGradient>
            </TouchableOpacity>
          </View>
        )}
        ...
        {sendEmailOtp && !isVerifyEmailOtp && (
          <View>
            <TouchableOpacity onPress={handleBackFormStepFour} style={styles.backBtn}>
              <Feather name="arrow-left" size={20} color="#2979ff" />
              <Text style={styles.backText}>Back</Text>
            </TouchableOpacity>

            <View style={styles.userIconCover}>
              <MaterialIcons name='mark-email-read' size={35} color="#2979ff" />
            </View>
            <Text style={styles.headerText}>Verify Email</Text>
            <Text style={styles.detailsText}>OTP sent to {email}</Text>

            <View style={styles.otpBoxRow}>
              {[emailOtp1Ref, emailOtp2Ref, emailOtp3Ref, emailOtp4Ref, emailOtp5Ref, emailOtp6Ref].map((ref, i) => (
                <TextInput
                  key={i}
                  ref={ref}
                  style={[styles.otpBox, { borderColor: emailOtp[i] ? '#2979ff' : '#2E3A59' }]}
                  maxLength={1}
                  keyboardType="numeric"
                  value={emailOtp[i]}
            ...
            {stepFourFormik.errors.emailOtp && stepFourFormik.touched.emailOtp && (
              <Text style={styles.errorText}>{stepFourFormik.errors.emailOtp}</Text>
            )}

            <TouchableOpacity disabled={isLoading} activeOpacity={0.4} onPress={stepFourFormik.handleSubmit}>
              <LinearGradient
                style={[styles.otpCover, (isLoading) && { opacity: 0.5 }]}
                colors={['#2979ff', '#7c3aed']}
                start={{ x: 0, y: 0 }}
              ...
              >
                {isLoading ? (<View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }} >
                  <ActivityIndicator size="small" color="#fff" style={{ marginRight: 10 }} />
                  <Text style={styles.otpBtn} >Verifying</Text>
                </View>) : <Text style={styles.otpBtn} >Verify & Finish</Text>}
              </LinearGradient>
            </TouchableOpacity>

            <TouchableOpacity disabled={!canResend} onPress={handleResendEmailOtp} style={{ marginTop: 16, alignItems: 'center' }}>
              <Text style={[styles.link, !canResend && { color: '#556080' }]}>
                {canResend ? 'Resend OTP' : `Resend in ${resendTimer}s`}
              </Text>
            </TouchableOpacity>
      ...
      {!signUp && (
        <View>
          <TouchableOpacity onPress={() => setSignUp(true)} style={styles.backBtn}>
            <Feather name="arrow-left" size={20} color="#2979ff" />
            <Text style={styles.backText}>Back</Text>
          </TouchableOpacity>
          ...
          <Text style={styles.formText}>Password</Text>
          <View style={[styles.passwordRow, { borderColor: isFocused === 'loginPassword' ? '#2979ff' : '#2A3550' }]}>
            <Feather name='lock' size={20} color="#888" />
            <TextInput
          ...
          <TouchableOpacity style={{ marginTop: 16, alignItems: 'center' }} onPress={() => setSignUp(true)}>
